...
### 待办
1. 简化发包流程: encoder自动打包, guid
    - 自动填充ownerGuid
    - 从传入channel, dpk, sender 到只需传入目标packet
    - 可简化响应包: 增加默认响应包: 包裹响应码与响应数据(可为空), 使用ppid与回调函数直接返回响应码无需内容
    - 可省略ppid, 增加(可省略的)回调参数, 并自动填充ppid
2. 考虑优化响应包: 封装一个父级响应包表示响应码, 使用回调来传递响应数据内容.
3. 增加心跳检测
4. 在服务端存储客户端信息字典:
   - 包括: 客户端Guid, 对应: 客户端InetSocketAddress, 玩家名字, 登录时长, 心跳计时
   - 在每个Packet携带OwnerGuid, 在Logic处理器判断客户端并处理逻辑  
5. 简化出栈步骤: 在客户端write出栈数据时, 通过Encoder处理器拦截并自动加上远程地址并编码信息.  